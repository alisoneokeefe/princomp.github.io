...
- C# also allows `finally` blocks, which contain code executed unconditionally, that is, regardless of whether the exception was thrown or not.
...
- A `try` block can have multiple `catch`es, as follows:
	...
	- Writing `catch{…}` is the same as `catch (Exception){…}`: by default, a `catch` block catches all the exceptions that can be thrown, not the exceptions of a particular class. Note that, if specifying multiple `catch` blocks, the order matters: a `catch (Exception)`, if placed first, will always execute before the `catch` blocks put after.
...
- The general rules are:
	...
	- Variables whose values are set in the `try` block will keep the values they had at the specific line where the `try` block throws an exception.
- For example, in the following code,
	...
	- If we were to remove the `zero = 0;` statement, then the program would display "The variable holds 2.". (This is because `zero = 2;` only executes if the line before it *does not* throw an exception.)
